...
fn get_optimal_buffer_size(file_path: &str) -> usize {
    ...
    } else if file_path.contains("$MFT")
        || file_path.contains("$LogFile")
        || file_path.contains("$UsnJrnl")
    {
    ...
    } else if file_path.contains("registry")
        || file_path.ends_with(".dat")
        || file_path.ends_with(".hive")
    {
...
pub fn collect_with_raw_handle(source_path: &str, dest_path: &Path) -> Result<ArtifactMetadata> {
    ...
    if handle == INVALID_HANDLE_VALUE {
        ...
        return Err(anyhow!(
            "Failed to open file with backup semantics: {}",
            err
        ));
    }
    ...
    let mut last_access_time = FILETIME {
        dwLowDateTime: 0,
        dwHighDateTime: 0,
    };
    ...
    let mut dest_file = File::create(dest_path).context(format!(
