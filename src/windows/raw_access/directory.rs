...
pub fn is_directory(path: &str) -> Result<bool> {
    ...
    let get_info_result =
        unsafe { winapi::um::fileapi::GetFileInformationByHandle(handle, &mut file_info) };
...
pub fn collect_directory(source_path: &str, dest_path: &Path) -> Result<ArtifactMetadata> {
    ...
    let find_handle =
        unsafe { winapi::um::fileapi::FindFirstFileW(wide_pattern.as_ptr(), &mut find_data) };
    ...
    loop {
        ...
        let filename = unsafe {
            let len = (0..260)
                .position(|i| find_data.cFileName[i] == 0)
                .unwrap_or(260);
        ...
        // Find next file
        let find_next_result =
            unsafe { winapi::um::fileapi::FindNextFileW(find_handle, &mut find_data) };
    ...
    debug!(
        "Collected directory with {} files, {} bytes",
