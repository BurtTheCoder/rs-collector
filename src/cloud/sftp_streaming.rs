...
impl SFTPUploadStream {
                    ...
                    match write_result {
                        ...
                        Err(e) => {
                            if attempts >= MAX_RETRIES {
                                return Err(anyhow!("Failed to write to remote file after {} attempts: {}", 
                                                 MAX_RETRIES, e));
                            }
                            
                            let delay = Duration::from_millis(250 * 2u64.pow(attempts as u32));
                            warn!("SFTP write attempt {} failed, retrying in {:?}: {}", 
                                 attempts, delay, e);
    ...
    pub async fn abort(self) -> Result<()> {
        ...
        match sftp_guard.unlink(Path::new(&self.remote_path)) {
            Ok(_) => {
                debug!("Aborted upload and removed remote file: {}", self.remote_path);
            },
            Err(e) => {
