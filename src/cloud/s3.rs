...
impl UploadQueue {
            ...
            match rusoto_credential::ProfileProvider::new() {
                Ok(mut provider) => {
                    ...
                    match rusoto_core::HttpClient::new() {
                        Ok(http_client) => {
                            Arc::new(S3Client::new_with(http_client, provider, region.clone()))
                        }
                        Err(e) => {
                ...
                Err(e) => {
                    warn!(
                        "Failed to create AWS profile provider: {}, using default",
                        e
                    );
                    Arc::new(S3Client::new(region.clone()))
                }
    ...
    pub async fn add_file(&self, file_path: PathBuf) -> Result<()> {
        ...
        let filename = file_path
            ...
            .ok_or_else(|| {
                anyhow!(
                    "Invalid file path - no filename component: {}",
                    file_path.display()
                )
            })?
            .to_string_lossy();
    ...
    async fn upload_small_file(&self, file_path: &Path, key: &str) -> Result<()> {
        ...
        file.read_to_end(&mut contents).context(format!(
            "Failed to read {} for S3 upload",
            file_path.display()
        ))?;
    ...
    async fn upload_large_file(&self, file_path: &Path, key: &str, file_size: u64) -> Result<()> {
        ...
        let _file_check = AsyncFile::open(file_path).await.context(format!(
            "Failed to open file for multipart upload: {}",
            file_path.display()
        ))?;
        ...
        for chunk_start in (1..=num_parts).step_by(concurrency_limit as usize) {
            ...
            for part_number in chunk_start..=chunk_end {
                ...
                let part_future = async move {
                    ...
                    Err(anyhow!(
                        "Failed to upload part {} after maximum retries",
                        part_number
                    ))
                };
...
pub async fn upload_files_concurrently(
    ...
    if uploaded < total {
        warn!(
            "Not all files were uploaded successfully: {}/{} bytes in region {}",
            uploaded, total, region_name
        );
    } else {
        info!(
            "All files uploaded successfully: {} bytes total in region {}",
...
pub async fn upload_to_s3(
    ...
    match result {
        Ok(_) => {
            ...
            info!(
                "Upload completed successfully: s3://{}/{}/{} in region {}",
...
mod tests {
    ...
    async fn test_multipart_upload_calculation() {
        ...
        for (file_size, expected_parts) in file_sizes {
            let num_parts = (file_size + UPLOAD_CHUNK_SIZE as u64 - 1) / UPLOAD_CHUNK_SIZE as u64;
            assert_eq!(
                num_parts, expected_parts,
                "File size {} should have {} parts",
    ...
    async fn test_upload_to_s3_legacy_nonexistent_file() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Failed to upload to S3"));
    }
