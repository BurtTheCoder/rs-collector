...
impl MemoryExporter {
    ...
    pub fn export_process_info(&self, process_info: &ProcessMemoryInfo) -> Result<PathBuf> {
        ...
        let process_dir = self
            .base_dir
            .join(format!("{}_{}", process_info.name, process_info.pid));

        // Create the directory if it doesn't exist
        fs::create_dir_all(&process_dir).context(format!(
            "Failed to create process directory: {}",
            process_dir.display()
        ))?;
        ...
        fs::write(&metadata_path, metadata_json).context(format!(
            "Failed to write process metadata to file: {}",
    ...
    pub fn export_memory_region(
        ...
        let filename = format!(
            "{}_{:x}_{:x}.dmp",
            region_type_str, region.base_address, region.size
        );

        let dump_path = process_dir.join(&filename);

        // Write the memory data to the file
        let mut file = File::create(&dump_path).context(format!(
            "Failed to create memory dump file: {}",
            dump_path.display()
        ))?;

        file.write_all(data).context(format!(
            "Failed to write memory data to file: {}",
            dump_path.display()
        ))?;
    ...
    pub fn export_summary(&self, summary: &MemoryCollectionSummary) -> Result<PathBuf> {
        ...
        fs::write(&summary_path, summary_json).context(format!(
            "Failed to write summary to file: {}",
            summary_path.display()
        ))?;

        debug!(
            "Exported memory collection summary to {}",
            summary_path.display()
        );
    ...
    pub fn create_memory_map(
        ...
        let mut file = File::create(&memory_map_path).context(format!(
            "Failed to create memory map file: {}",
            memory_map_path.display()
        ))?;

        // Write header
        writeln!(
            file,
            "Address Range                Size       Type       Permissions  Name"
        )?;
        writeln!(
            file,
            "-------------------------- ------------ ---------- ------------ ----------------"
        ...
        for region in regions {
            ...
            writeln!(
                file,
                "{:016x}-{:016x} {:12} {:10} {:12} {}",
                region.base_address, end_address, region.size, region_type, permissions, name
            )?;
        }
    ...
    pub fn create_collection_summary(
        ...
        let total_memory_collected = processes.iter().map(|p| p.dumped_memory_size).sum();
