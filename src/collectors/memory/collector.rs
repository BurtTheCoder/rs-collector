...
use crate::constants::{DEFAULT_MAX_PROCESS_MEMORY, DEFAULT_MAX_TOTAL_MEMORY};
...
impl MemoryCollector {
    ...
    pub fn from_args(
        ...
        let process_filter =
            ProcessFilter::from_args(process_names, process_ids, include_system_processes);
    ...
    pub fn collect_all(
        ...
        for process in filtered_processes {
            ...
            match self.collect_process(process, &exporter) {
                ...
                Err(e) => {
                    warn!(
                        "Failed to collect memory from process {}: {}",
                        process.pid, e
                    );
        ...
        let summary =
            MemoryExporter::create_collection_summary(&process_infos, start_datetime, end_datetime);
    ...
    fn collect_process(
        ...
        if total_memory_size > self.options.max_process_size {
            warn!(
                ...
                pid, total_memory_size, self.options.max_process_size
            );
        ...
        let process_dir = exporter
            .export_process_info(&process_info)
        ...
        for region in &mut process_info.regions {
            ...
            match self
                .platform_impl
                .read_memory(pid, region.base_address, region.size as usize)
            {
                Ok(data) => {
                    if data.is_empty() {
                        debug!(
    ...
    pub fn search_memory(&self, process: &ProcessInfo, pattern: &[u8]) -> Result<Vec<u64>> {
        info!(
            "Searching for pattern in process {} ({})",
    ...
    pub fn scan_memory_yara(&self, process: &ProcessInfo, rules: &[&str]) -> Result<Vec<String>> {
        info!(
            "Scanning process {} ({}) with YARA rules",
            process.name, process.pid
        );
    ...
    pub fn dump_memory_region(
        ...
        info!(
            "Dumping memory region at {:x} (size: {}) for process {} ({})",
...
mod tests {
    ...
    fn test_process_filter_creation() {
        let filter = ProcessFilter::from_args(Some("firefox,chrome"), Some("1234,5678"), true);
    ...
    fn test_memory_region_filter() {
        let filter = MemoryRegionFilter::from_str("heap,stack", 4096, DEFAULT_MAX_TOTAL_MEMORY);
    ...
    fn test_memory_collection_summary_creation() {
        ...
        let summary = MemoryExporter::create_collection_summary(&processes, start_time, end_time);
