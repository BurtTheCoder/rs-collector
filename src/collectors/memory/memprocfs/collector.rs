...
use crate::collectors::memory::models::{
    MemoryRegionInfo, MemoryProtection, ModuleInfo,
};
#[cfg(feature = "memory_collection")]
...
impl MemoryCollectorImpl for MemProcFSCollector {
    ...
    fn get_memory_regions(&self, process: &ProcessInfo) -> Result<Vec<MemoryRegionInfo>> {
        #[cfg(feature = "memory_collection")]
        {
            ...
            let proc = vmm.process_from_pid(pid)
            ...
            for vad in &*vad_map {
                ...
                let region = MemoryRegionInfo {
                    ...
                    mapped_file: if !vad.info.is_empty() { Some(vad.info.clone()) } else { None },
                    dumped: false,
                    dump_path: None,
                };
    ...
    #[cfg(feature = "yara")]
    fn scan_memory_yara(&self, _pid: u32, _rules: &[&str], _start_addr: u64, _end_addr: Option<u64>) -> Result<Vec<String>> {
        ...
        Err(anyhow!("YARA scanning not yet implemented - get_process method missing"))
    }
}
