...
impl MemoryCollectorImpl for LinuxMemoryCollector {
    fn new() -> Result<Self> {
        ...
        if !Path::new("/proc").exists() {
            bail!("Cannot access /proc filesystem. Memory collection requires /proc to be mounted.");
        }
    ...
    fn get_memory_regions(&self, process: &ProcessInfo) -> Result<Vec<MemoryRegionInfo>> {
        ...
        for line in maps_content.lines() {
            ...
            if addrs.len() != 2 {
                warn!("Invalid address range format in /proc/{}/maps: {}", pid, addr_range);
                continue;
            }
    ...
    fn get_modules(&self, process: &ProcessInfo) -> Result<Vec<ModuleInfo>> {
        ...
        for line in maps_content.lines() {
            ...
            if !path.ends_with(".so") && !path.contains(".so.") && !path.contains("/bin/") && !path.contains("/lib/") {
                continue;
            }
