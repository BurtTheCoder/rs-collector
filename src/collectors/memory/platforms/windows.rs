...
impl MemoryCollectorImpl for WindowsMemoryCollector {
    fn new() -> Result<Self> {
        #[cfg(feature = "memory_collection")]
        {
            ...
            let vmm = Vmm::initialize(None).context("Failed to initialize MemProcFS")?;
    ...
    fn get_memory_regions(&self, process: &ProcessInfo) -> Result<Vec<MemoryRegionInfo>> {
        #[cfg(feature = "memory_collection")]
        {
            ...
            let vmm_process = self
                .vmm
                .process_get(pid as u32)
                .context(format!("Failed to get process {} from MemProcFS", pid))?;

            // Get memory map
            let memory_map = vmm_process
                .map_vad()
            ...
            for entry in memory_map {
                ...
                } else if entry.protection & 0x10 != 0 {
                    // IMAGE flag
    ...
    fn read_memory(&self, pid: u32, address: u64, size: usize) -> Result<Vec<u8>> {
        #[cfg(feature = "memory_collection")]
        {
            ...
            let bytes_read = vmm_process.mem_read(address, &mut buffer).context(format!(
