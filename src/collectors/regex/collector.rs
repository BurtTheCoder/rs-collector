        ...
        let regex_config = match &artifact.regex {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "Regex config not enabled for this artifact"
                ))
            }
        };

        info!(
            "Collecting regex artifact: {} from {}",
            artifact.name,
            source_base.display()
        );
...
mod tests {
    ...
    async fn test_collect_with_regex_not_enabled() {
        ...
        let result = collector
            .collect_with_regex(&artifact, temp_dir.path(), temp_dir.path())
            .await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Regex config not enabled"));
    }
    ...
    async fn test_collect_with_regex_simple_pattern() {
        ...
        let result = collector
            .collect_with_regex(&artifact, &source_dir, &output_dir)
            .await;
