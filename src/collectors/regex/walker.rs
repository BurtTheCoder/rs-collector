...
impl<'a> DirectoryWalker<'a> {
    ...
    pub fn new(
        ...
        let include_regex = Regex::new(include_pattern)
            .context("Invalid include pattern regex")?;
    ...
    fn walk_directory_recursive(
        ...
        if !current_path.exists() {
            return Err(anyhow::anyhow!("Path does not exist: {}", current_path.display()));
        }
        
        if !current_path.is_dir() {
            return Err(anyhow::anyhow!("Path is not a directory: {}", current_path.display()));
        }
