...
pub fn create_destination_path(source_path: &Path, base_path: &Path, output_base: &Path) -> Result<PathBuf> {
    // Get path relative to base
    let relative_path = source_path.strip_prefix(base_path)
        .context(format!("Failed to get relative path for {}", source_path.display()))?;
