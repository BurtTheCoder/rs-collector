...
impl MacOSCollector {
    ...
    fn collect_unified_logs(&self, source: &Path, dest: &Path) -> Result<ArtifactMetadata> {
        ...
        fs::write(dest, output.stdout).context(format!(
            "Failed to write unified logs to {}",
            dest.display()
        ))?;

        // Get file metadata
        let metadata =
            fs::metadata(dest).context(format!("Failed to get metadata for {}", dest.display()))?;
            ...
            match &artifact_type {
                ...
                _ => {
                    ...
                    if source_path_clone.is_dir() {
                        collector
                            .fallback
                            .collect_directory(&source_path_clone, &output_path_clone)
                    } else {
                        collector
                            .fallback
                            .collect_standard_file(&source_path_clone, &output_path_clone)
                    }
                }
            }
        })
...
mod tests {
    ...
    fn test_macos_collector_new() {
        ...
        assert!(
            collector.supports_artifact_type(&ArtifactType::MacOS(MacOSArtifactType::UnifiedLogs))
        );
    }
    ...
    fn test_supports_artifact_type() {
        ...
        assert!(
            collector.supports_artifact_type(&ArtifactType::MacOS(MacOSArtifactType::Spotlight))
        );
        assert!(
            collector.supports_artifact_type(&ArtifactType::MacOS(MacOSArtifactType::Quarantine))
        );
        assert!(
            collector.supports_artifact_type(&ArtifactType::MacOS(MacOSArtifactType::KnowledgeC))
        );
        assert!(
            collector.supports_artifact_type(&ArtifactType::MacOS(MacOSArtifactType::LaunchAgents))
        );
        assert!(collector
            .supports_artifact_type(&ArtifactType::MacOS(MacOSArtifactType::LaunchDaemons)));
        ...
        // Unsupported types
        assert!(!collector.supports_artifact_type(&ArtifactType::Windows(
            crate::config::WindowsArtifactType::MFT
        )));
        assert!(!collector.supports_artifact_type(&ArtifactType::Linux(
    ...
    async fn test_collect_macos_artifact_fsevents() {
        ...
        fs::write(
            test_fsevents_dir.join("0000000000000001"),
            "fake fsevents data\n",
        )
        .unwrap();
...
<dict>
    ...
    async fn test_collect_launch_agents_directory() {
        ...
        fs::write(
            launch_agents_dir.join("com.test.agent1.plist"),
            "agent1 content\n",
        )
        .unwrap();
        fs::write(
            launch_agents_dir.join("com.test.agent2.plist"),
            "agent2 content\n",
        )
        .unwrap();
