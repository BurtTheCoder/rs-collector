...
pub fn build_binary_with_config(
    ...
    fs::copy(config_path, &os_config_path).context(format!(
    ...
    fs::copy(config_path, &generic_config_path).context(format!(
        "Failed to copy config to {}",
        generic_config_path.display()
    ))?;

    info!(
        "Building for {} with target {}",
        target_os_normalized, target_triple
    );
    ...
    let source_path = format!(
        "target/{}/release/rust_collector{}",
        target_triple,
        if target_os_normalized == "windows" {
            ".exe"
        } else {
            ""
        }
    ...
    fs::copy(&source_path, &output_file).context(format!(
    ...
    info!(
        "Build completed successfully. Binary available at: {}",
        output_file.display()
    );
    Ok(output_file)
}
...
pub fn execute_build_script(script_path: &Path) -> Result<()> {
    ...
    warn!(
        "The path provided ({}) is not a script but will be treated as a binary",
        script_path.display()
    );
