...
use rust_collector::{
    collectors::{
        ...
        volatile::{collect_processes, collect_volatile_data},
    },
...
use rust_collector::collectors::memory::{
    ...
    filters::{MemoryRegionFilter, ProcessFilter},
...
fn benchmark_compression(test_dir: &Path) -> Result<BenchmarkResult> {
    ...
    for count in file_counts {
        ...
        println!(
            "  {} files: {:?} (compression: {:.1}%)",
            count, duration, compression_ratio
        );
...
fn benchmark_bodyfile_generation(test_dir: &Path) -> Result<BenchmarkResult> {
    ...
    for count in file_counts {
        ...
        options.insert(
            "root_path".to_string(),
            bodyfile_dir.to_string_lossy().to_string(),
        );
...
fn benchmark_memory_collection(test_dir: &Path, rt: &Runtime) -> Result<BenchmarkResult> {
    ...
    let current_process = processes.iter().find(|p| p.pid == std::process::id());
