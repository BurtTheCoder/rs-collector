...
use crate::constants::{
    COMPRESSED_EXTENSIONS, COMPRESSION_CHUNK_SIZE as CHUNK_SIZE, LARGE_FILE_COMPRESSION_THRESHOLD,
};
...
/// # Ok::<(), anyhow::Error>(())
/// ```
pub fn compress_artifacts(source_dir: &Path, hostname: &str, timestamp: &str) -> Result<PathBuf> {
    let start = Instant::now();
    ...
    // Create zip file
    let zip_file = fs::File::create(&zip_path).context("Failed to create zip file")?;
    ...
    let workers: Result<Vec<_>> = (0..thread_count)
        .map(|i| {
...
mod tests {
    ...
    fn test_compress_artifacts_basic() {
        ...
        for expected in expected_files {
            let found = (0..archive.len()).any(|i| archive.by_index(i).unwrap().name() == expected);
    ...
    fn test_compress_artifacts_with_subdirectories() {
        ...
        fs::write(
            base_path.join("level1/level2/level3/file3.txt"),
            b"level 3 file",
        )
        .unwrap();
    ...
    fn test_compression_worker_basic() {
        ...
        test_file
            .write_all(b"Test content for compression")
            .unwrap();
