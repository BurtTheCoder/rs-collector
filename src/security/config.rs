...
impl SecurityConfig {
    ...
    pub fn is_output_dir_allowed(&self, path: &std::path::Path) -> bool {
        if self.allowed_output_dirs.is_empty() {
            ...
            self.allowed_output_dirs.iter().any(|allowed| {
...
pub fn log_security_event(event: SecurityEvent) {
    ...
    match event {
        SecurityEvent::PathValidationFailed { path, reason } => {
            warn!("Security: Path validation failed for '{}': {}", path, reason);
        }
        ...
        SecurityEvent::SensitiveFileAccess { path, action } => {
            warn!("Security: Sensitive file access - Action: '{}' Path: '{}'", action, path);
        }
        SecurityEvent::PolicyViolation { policy, details } => {
            warn!("Security: Policy violation - Policy: '{}' Details: {}", policy, details);
        }
        SecurityEvent::Authentication { method, success } => {
            if success {
                warn!("Security: Authentication successful using method '{}'", method);
            } else {
...
mod tests {
    ...
    fn test_is_output_dir_allowed() {
        ...
        // Add restrictions
        config.allowed_output_dirs = vec![
            PathBuf::from("/tmp"),
            PathBuf::from("/home/user/forensics"),
        ];
