...
use rust_collector::security::path_validator::{
    sanitize_filename, validate_output_path, validate_path,
};
use std::path::Path;
...
fn bench_path_validation(c: &mut Criterion) {
    ...
    for (name, path) in test_paths {
        group.bench_with_input(BenchmarkId::new("validate_path", name), path, |b, path| {
            b.iter(|| {
                let _ = validate_path(black_box(Path::new(path)), black_box(Some(base_path)));
            });
        });
    }
        ...
        group.bench_with_input(
            ...
            |b, paths| {
                b.iter(|| {
                    for path in paths {
                        let _ =
                            validate_path(black_box(Path::new(path)), black_box(Some(base_path)));
                    }
                });
            },
        );
